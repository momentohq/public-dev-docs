...
Momentoは[堅牢な認証メカニズム](/topics/authentication/tokens)を提供し、コンシューマが特定のリソースやアクションにアクセスできるように制限します。インスタントメッセージ機能を構築するとき、ユーザーがアクセスすべきではないメッセージを傍受しないように、スコープを考慮することは非常に重要です。
