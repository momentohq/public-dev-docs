...
インスタントメッセージングは、多くのアプリケーションで必須の機能です。1対1、1対多、多対多の会話をシームレスに行うことは、多くの場合、アプリケーションに必要なコンポーネントと考えられています。Momento CacheとTopicsを使用すると、バックエンドのインフラストラクチャなしで、アプリケーションにインスタント・メッセージングを組み込むことができます。[Momento web SDK](../../sdks/web)を使用すると、ブラウザで直接キャッシュデータにアクセスし、メッセージを公開および購読できます。
