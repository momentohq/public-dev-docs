---
sidebar_position: 2
sidebar_label: レートリミッターの構築
title:  Momentoを使った分散型レートリミッターの実装
description: 一元化されたバックエンドソース、Momentoを使用して分散型レートリミッターを実装する方法をご紹介します。
---

import { SdkExampleCodeBlock } from "@site/src/components/SdkExampleCodeBlock";
// inject-example-code-snippetプラグインは、SdkExampleCodeBlockのインスタンスをSdkExampleCodeBlockImplに変換します。
...
// inject-example-code-snippetプラグインは、SdkExampleCodeBlockのインスタンスをSdkExampleCodeBlockImplに変換します。
// プラグインは、SdkExampleFileTabsのインスタンスをSdkExampleFileTabsImplに変換します。
...
私たちのアプローチは、Momentoの`increment`と`updateTTL` [API](./../develop/api-reference)を利用する。この方法は効率的であるだけでなく、非常に正確であることが証明されています。
...
早速、実装に取りかかましょう。このコードでは、思考プロセスを説明するコメントに注目してください。
...
- [Read our blog](https://www.gomomento.com/blog/did-you-say-you-want-a-distributed-rate-limiter) ここでは、レートリミッターのさまざまなヒューリスティックを分析し、他のアプローチとも比較しています。
