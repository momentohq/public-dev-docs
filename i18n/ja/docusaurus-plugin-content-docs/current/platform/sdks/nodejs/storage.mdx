...
Momentoは2つのJavaScript SDKを提供しています。[1つはNode.js用](/platform/sdks/nodejs)、[1つはブラウザその他のWebアプリケーション用](/platform/sdks/web)です。2つのSDKは同じAPIを持っているため、`import`文以外、コードは同じように見えますが、その裏側では、異なるJavaScript実行環境で最適なパフォーマンスと互換性を実現するために構築されています。
