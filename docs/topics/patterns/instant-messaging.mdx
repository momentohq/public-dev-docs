...
Momento provides a [robust authorization mechanism](../authentication/tokens/index.md) that limits consumers to specific resources and actions. When building an instant messaging feature, it is incredibly important to take scope into consideration so users do not intercept messages they shouldn't have access to.
