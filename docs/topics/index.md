...
- **[Notifying users on updates](https://github.com/momentohq/client-sdk-javascript/tree/main/examples/nodejs/aws/eventbridge)** - Use both Momento Topics and Cache along with Amazon DynamoDB and DynamoDB Streams to notify users automatically when data is updated in your database. This example uses EventBridge API Destinations to connect directly to Momento services when a data filter is matched on in a stream.
