...
# Namespace and item quotas in Momento Leaderboards
...
As an example, an `Upsert` request adding one or two elements costs *one operation*, but with three or four elements it costs *two operations*, and so on.
