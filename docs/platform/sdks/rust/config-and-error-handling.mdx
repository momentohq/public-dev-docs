...
- `CredentialProvider::from_default_env_var_v2` - reads the API key and endpoint from environment variables MOMENTO_API_KEY and MOMENTO_ENDPOINT
- `CredentialProvider::from_env_var_v2` - reads the API key from the provided environment variable names
- `CredentialProvider::from_api_key_v2` - takes the API key and endpoint as strings; can be used when retrieving the key from a secret manager, etc.
- `CredentialProvider::from_disposable_token` - takes a disposable token or v1 API key as a string; can be used when retrieving the key from a secret manager, etc.
