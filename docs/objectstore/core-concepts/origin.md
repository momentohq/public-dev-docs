...
1. **Storage** - Store video, audio, and manifest files, typically in formats for [adaptive bitrate streaming](/objectstore/performance/adaptive-bitrates/how-it-works)
2. **Delivery** - Retrieve media [segments](/objectstore/core-concepts/segments) and manifests and deliver them to a CDN
...
* **Video encoder** - Transforms raw video/audio files into formats suitable for streaming, like [HLS (HTTP Live Streaming)](/objectstore/performance/adaptive-bitrates/hls) or [DASH (Dynamic Adaptive Streaming over HTTP)](/objectstore/performance/adaptive-bitrates/dash)
...
## Momento Object Store as an origin

Momento Object Store is a purpose-built high-speed origin designed to meet the demands of [live streaming](/objectstore/streaming/live-streaming/how-it-works) and is also able to transfer the live content to a long term storage tier for [video-on-demand (VOD)](/objectstore/streaming/video-on-demand/object-storage) applications. By leveraging our ultra-low latency storage, the Momento Object Store origin ensures content is always accessible and ready for distribution to CDNs, enabling a smooth, uninterrupted viewer experience. Whether it's live events or on-demand content, Momento Object Store simplifies the process of managing media delivery while maintaining exceptional performance.

Reliability is at the core of all services on the Momento platform, and the Object Store product is no exception. It minimizes buffering, achieving a [zero-buffer rate (ZBR)](/objectstore/core-concepts/zero-buffer-rate) for viewers across devices and locations. The platform is built to scale effortlessly, handling concurrent requests from multiple CDNs without interruption, making it ideal for high-traffic events like live sports.

The developer-friendly API makes integrating and managing media assets straightforward, reducing complexity in workflows. With dynamic, automatic scaling of our services, Object Store consistently delivers content even during traffic surges or outages, allowing video streaming engineering teams to focus on building and innovating without worrying about infrastructure. This combination of reliability and simplicity provides a dependable and easy-to-use solution for modern media streaming.
