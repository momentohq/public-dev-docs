...
sidebar_position: 1
sidebar_label: RTMP
title: RTMP and Momento Object Store
description: Learn how to use RTMP to capture and transcode live video and upload it to Momento Object Store.
hide_title: true
keywords:
  - momento
  - media storage
...
Now that the request is handled, we need to write the async workflow that ingests the RTMP stream and transcodes it to [different bitrates and resolutions](/objectstore/core-concepts/abr-ladder).
...
Next, we need to implement the watcher function that uploads segments to Momento Object Store as they are created in real time.
...
5. **Monitor transcoding and uploading**
    - The transcoding process will begin, with your app creating segments and uploading them to **Momento Object Store** in real-time. The server will log the upload process and notify you when it's complete.
    - You can also view the generated segments on your file system as they are created in each resolution folder

5. **Playback**
    - Once the segments and manifest files are in **Momento Object Store**, they can be accessed via an HLS-compatible player using the master playlist URL:
    ```
...
:::info
The method we used in this demo was the [CDN route](/objectstore/streaming/decoding-video#using-a-cdn-with-header-forwarding) for accessing media files. This CDN takes requests and forwards a Momento auth token directly to Momento Object Store. If you do not have a CDN configured to do this, the media player will be unable to fetch the manifest and segments.
:::

You are now ready to handle RTMP live streaming, transcoding, and storing segments in **Momento Object Store**. You can adjust the stream quality, segments length, and other parameters based on your streaming needs. Happy coding!
