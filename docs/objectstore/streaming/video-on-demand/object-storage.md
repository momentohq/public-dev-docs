...
With video on demand (VOD) workflows, media storage plays a critical role in making pre-recorded content readily available to viewers whenever they choose to watch. Efficient storage allows fast access and scalability, making it possible for platforms to serve millions of users without delays, especially during high-traffic periods. **Momento Object Store** provides an optimized solution for VOD, offering ultra-low latency access to video segments and manifest files, ensuring seamless playback across devices.

## Storage characteristics

The media storage system in a VOD workflow must support several key tasks:

* **Storing video segments** - When content is prepared for VOD, it is encoded and divided into small chunks called [segments](/objectstore/core-concepts/segments) which are stored on the [origin](/objectstore/core-concepts/origin). **Momento Object Store** acts as the origin, offering in-memory storage that enables ultra-fast retrieval of video segments whenever they are requested by a content delivery network (CDN).

* **Managing manifest files** - In addition to storing video segments, the media storage system manages **manifest files** (e.g., `.m3u8` for [HLS](/objectstore/performance/adaptive-bitrates/hls) or `.mpd` for [DASH](/objectstore/performance/adaptive-bitrates/dash)), which contain the [available bitrates and resolutions](/objectstore/core-concepts/abr-ladder) of the video. These manifest files guide the video player in selecting the appropriate segments based on network conditions.

* **Handling multiple resolutions and bitrates** - VOD content is often encoded into multiple bitrates and resolutions to accommodate different network conditions and device capabilities. **Momento Object Store** stores the segments of each version of the video as individual items, supporting [adaptive bitrate streaming (ABR)](/objectstore/performance/adaptive-bitrates/how-it-works) workflows by ensuring that each segment is delivered quickly, whether in high-definition or lower-quality formats.
