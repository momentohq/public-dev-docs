...
Decoding video streams allows video players to stream content, whether it's [live streaming](/objectstore/streaming/live-streaming/how-it-works) or [video on demand (VOD)](/objectstore/streaming/video-on-demand/object-storage). The process involves fetching [segments](/objectstore/core-concepts/segments) from a content delivery network (CDN), decoding them into viewable content, and delivering the video to the viewer's screen with minimal delay. Two widely used protocols dominate video streaming: [HLS (HTTP Live Streaming)](/objectstore/performance/adaptive-bitrates/hls) and [DASH (Dynamic Adaptive Streaming over HTTP)](/objectstore/performance/adaptive-bitrates/dash). These protocols rely on *manifest files* to point the player at the location of segments at the [desired bitrate and resolution](/objectstore/core-concepts/abr-ladder).

## How video players work

As mentioned ealier, the two main protocols in video streaming are HLS and DASH. Both protocols enable support for [adaptive bitrate streaming (ABR)](/objectstore/performance/adaptive-bitrates/how-it-works), allowing the player to switch between different resolutions and bitrates dynamically to optimize playback quality based on the viewer's network conditions. This is done through the use of **manifest files**.

### Manifest files and segments stored in Momento Object Store

Manifest files list available video segments for a stream, along with their corresponding bitrates and resolutions. These manifest files direct the video player to the specific segments and other manifests stored in **Momento Object Store**.

Stored segments and bitrate/resolution specific manifest files are identified by **key names**. The key names map either directly to the url where the segment lives or to a relative path based on the base URL of the manifest. Files stored in Momento Object Store are accessible via HTTP based on the namespace and name of the key.
...
Live streams are short-lived by nature. As content is recorded, it is streamed to end users in the moment. After it plays, it's over and the segments aren't accessible unless the provider decides to make it available on demand. This use case is perfect for a [Momento session token](/platform/authentication/tokens), which is created with a finite duration and limited scope permission set.

Tokens used for live streaming should be generated with `readonly` access scoped to either a specific namespace or [key prefix](/platform/authentication/roles-and-permissions#item-level-restriction) to operate with least-privilege permissions. The path to segments or manifest files should match one of the examples below.
...
In video on demand (VOD) workflows, requests for access to segments could come at any point in time, so using a short-lived token is not a possible solution. Instead, using a CDN like Amazon CloudFront or Cloudflare to forward your request with added headers is a viable alternative using a long-lived [Momento API key](/platform/authentication/api-keys). This solution allows video players to make calls do your CDN without knowledge of a token or API key and have it automatically added for them on the server side.
...
* Each entry points to a media playlist in Momento Object Store for that specific quality level using relative paths
