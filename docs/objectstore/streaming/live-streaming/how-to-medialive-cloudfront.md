...
description: Using Momento Object Store as a live HLS origin for AWS MediaLive and CloudFront.
...
# Creating a live video origin with Momento Object Store, AWS MediaLive, and Amazon CloudFront

Live video streaming requires a highly performant and scalable [origin server](/objectstore/core-concepts/origin) to handle the demands of a global audience. Traditional origin servers often struggle with latency, scalability, or complexity when integrated with modern cloud workflows. Momento Object Store offers a simple, high-scale solution by providing a robust, low-latency, and cost-effective origin solution tailored for live video streaming.

In this tutorial, you will learn how to set up Momento Object Store as the origin server for a live video stream encoded by [AWS Elemental MediaLive](https://aws.amazon.com/medialive/) and delivered via [Amazon CloudFront](https://aws.amazon.com/CloudFront/). By the end of this guide, you'll have a fully functional workflow capable of delivering live [HLS streams](/objectstore/performance/adaptive-bitrates/hls) optimized for performance and reliability.

Here's what we'll cover:

1. Creating a storage namespace in Momento Object Store to act as the origin server.
...
Momento Object Store supports a maximum object size of 10MB. To estimate segment size, divide the bitrate in Kbps by 8, multiply by the segment duration in seconds, and add 10% to account for headers. For example, a 2 second segment at 1000 Kbps is roughly `1000 / 8 * 2 * 1.1 = 275 KB`.

:::

For the purposes of this tutorial, configure one live video channel for an [adaptive bitrate ladder](/objectstore/core-concepts/abr-ladder) with 3 encoding profiles and 2.5-second segments:
...
While Momento Object Store is in private preview, the Storage *namespace* will be appear as a *cache* in the console. This guide will use *namespace* interchangeably with *cache*. As Momento Object Store approaches general availability, updates to the console will make namespace creation a self-service process.
...
Video players read the content pushed by encoders to Momento Object Store via a **Content Delivery Network (CDN)**. As a security best practices, create a second API key with *readonly* access to the `live-origin` namespace. Use the same [API key page](https://console.gomomento.com/api-keys) as before to create this new API key. The guide will refer to this API key as `player_api_key`.
...
AWS Elemental MediaLive is a live video encoding service from AWS that processes and delivers live streams at scale. MediaLive converts live video into streaming formats like [HLS](/objectstore/performance/adaptive-bitrates/hls). It is commonly used for live sports, major live events, and 24/7 channels.

MediaLive will act as the **encoder** in this live video workflow. The encoded video will be sent directly to Momento Object Store, which functions as the origin server.
...
Amazon CloudFront is a Content Delivery Network (CDN) that ensures the live video streams are delivered quickly and reliably to viewers worldwide. In this step, we'll configure CloudFront to work with Momento Object Store - optimizing latency, managing costs, and securing access.
...
[Media segments](/objectstore/core-concepts/segments) are assets that change infrequently. They represent 1-5 seconds of video, audio, or metadata that will not change once it's been generated by the encoder. These media assets are usually best cached by a CDN service like Amazon CloudFront across multiple PoPs located close to the end viewers for both [live](/objectstore/streaming/live-streaming/how-it-works) and [VOD](/objectstore/streaming/video-on-demand/object-storage) video workflows.
...
The three commands below produce the same results as the requests sent from the player through CloudFront to Momento Object Store. Remember that the media segments listed in each variant playlist constantly change, so replace `<id>` in the final command with a valid value from the fetched playlist:
...
Congratulations! You've successfully set up a live video streaming workflow using Momento Object Store, AWS MediaLive, and Amazon CloudFront. This modern, scalable solution ensures low latency, reliability, and optimal performance for delivering live HLS streams.

**Here's what you've learned:**

* Configured Momento Object Store as a highly performant and cost-effective origin server.
...
* Learn about [adding entitlements](/objectstore/entitlements/about) to your workflow.
* Discover [how to use ffmpeg and an RTMP stream](/objectstore/streaming/capture/rtmp) to stream to Momento.
* Add [real-time reactions](/objectstore/enhancements/live-reactions) to your streams.
