...
:::warning
Do NOT use compare-and-set (CAS) APIs such as `SetIfAbsent` with scalar or non-CAS APIs such as `Set`. Doing so will result
in undefined consistency behaviors. Use `SetIfPresent` instead with `SetIfAbsent`.
:::
...
:::warning
Do NOT use compare-and-set (CAS) APIs such as `SetIfPresent` with scalar or non-CAS APIs such as `Set`. Doing so will result
in undefined consistency behaviors. Use `SetIfAbsent` instead with `SetIfPresent`.
:::
