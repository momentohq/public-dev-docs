...
<!-- Projects: cache2/constraints/service-limits, cache2/concepts/capacity-and-usage, cache2/capabilities/onboarding-flow -->
...
| Scope | Default limit |
| --- | ---: |
| Total available capacity per account | 500 GiB |
...
The 500 GiB limit is an admission limit on available capacity. Available capacity is the configured
Valkey `maxmemory` on each primary shard multiplied by the number of primary shards. Replicas do
not add available capacity.
