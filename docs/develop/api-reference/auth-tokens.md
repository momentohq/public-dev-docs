...
The auth APIs create and manage authorization tokens for Momento services, known as [Momento auth tokens](./../guides/working-with-momento-auth-tokens). Tokens can be scoped to have one or more permissions to grant access to one or more caches or topics.
