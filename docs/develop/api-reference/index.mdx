...
<SdkExamples
  js={`
const { SimpleCacheClient, EnvMomentoTokenProvider, LogLevel, LogFormat, Configurations } = require('@gomomento/sdk');
const credentials = new StringMomentoTokenProvider ({ authToken: process.env.MOMENTO_AUTH_TOKEN});\n
...
_ITEM_DEFAULT_TTL_SECONDS = 15\n
with scc.SimpleCacheClient(_MOMENTO_AUTH_TOKEN, _ITEM_DEFAULT_TTL_SECONDS) as cache_client:
    cache_client.flush_cache('test-cache')
`}
  java={`
String MOMENTO_AUTH_TOKEN = "eyJhbGc.MyTestToken";
int DEFAULT_ITEM_TTL_SECONDS = 15;\n
SimpleCacheClient simpleCacheClient = SimpleCacheClient
    .builder(MOMENTO_AUTH_TOKEN, DEFAULT_ITEM_TTL_SECONDS)
    .build()\n
simpleCacheClient.flushCache("test-cache");
    `}
  go={`
const (
    authToken             = "eyJhbGc.MyTestToken"
...
err = client.FlushCache(&momento.FlushCacheRequest{
    CacheName: "test-cache",
})
    `}
  csharp={`
...
ICredentialProvider authProvider = new StringMomentoTokenProvider("eyJhbGc.MyTestToken");
using SimpleCacheClient client = new SimpleCacheClient(Configurations.Laptop.Latest(), authProvider, DEFAULT_TTL);
await client.FlushCacheAsync("test-cache");
    `}
  rust={`
let auth_token = "eyJhbGc.MyTestToken";
let item_default_ttl_seconds = 15;
...
response = client.flush_cache('test-cache')
raise repsonse.error if response.error?
  `}
  cli={`momento cache flush-cache --name test-cache`}
/>
...
* [Dictionaries](./dictionary-collections.md) are used to store unordered key:value pairs.
* [Lists](./list-collections.md) are a collection of ordered values, sorted in the sequence each element was inserted.
* [Sets](./set-collections.md) are an unordered collection of unique strings.
